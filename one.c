...
int init_atlas()/*{{{*/
{
	SDL_Surface *sur = SDL_LoadBMP("tiles.bmp");
...
int process_logic()/*{{{*/
{
	...
	for(i = 0; i < ENEMIES; i++)
	{
		...
		if(monster[i].rect.y > player.rect.y)
			monster[i].rect.y -= hs / 16;
		if(monster[i].rect.x > player.rect.x)
			monster[i].rect.x -= hs / 16;
		if(monster[i].rect.x < player.rect.x)
			monster[i].rect.x += hs / 16;
		if(monster[i].rect.y < player.rect.y)
			monster[i].rect.y += hs / 16;
...
int main(int argc, char *argv[])
{
	...
	mouse.rect.h = SCALE;
	mouse.firing = false;
	hpack.id = health_plus;
	hpack.sprite = plus;
	hpack.rect.x = (rand() % gridsize.x) * SCALE;
	hpack.rect.y = (rand() % gridsize.y) * SCALE;
	hpack.rect.w = SCALE;
	hpack.rect.h = SCALE;
	shpack.id = shells_plus;
